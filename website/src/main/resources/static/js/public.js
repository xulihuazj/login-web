...
$.HTTP.URL = {
    ...
    MOVIE_RECOMMEND_URL: '/movie/list',
...
$.STORAGE.Check = function () {
    if (localStorage) {
        ...
        console.error("localstroage 启用失败");
        return false;
    }
...
$.STORAGE.COOKIE = {}
$.STORAGE.COOKIE.Put = function (k, v) {
    Cookies.set(k, v, {expires: 365});
}
$.STORAGE.COOKIE.Get = function (k) {
    return Cookies.get(k);
}
$.STORAGE.COOKIE.Remove = function (k) {
    Cookies.remove(k);
}

$.COMMON.DATA = {};
$.COMMON.DATA.CITY = 2221;
$.COMMON.getCity = function () {
    var cityId = $.COMMON.DATA.CITY,
        city = $.STORAGE.Get($.STORAGE.KEY.CITY),
        cookieCity = $.STORAGE.COOKIE.Get($.STORAGE.KEY.CITY)
    ;
    ...
    $.STORAGE.Put($.STORAGE.KEY.CITY, cityId);
    $.STORAGE.COOKIE.Put($.STORAGE.KEY.CITY, cityId);
